...
public class FormatRule implements Rule<JType, JType> {
     ...
     * <ul>
     * <li>"format":"date-time" =&gt; {@link Date} or {@link DateTime} (if config useJodaDates is set)
...
//        mapping.put("uri", URI.class);
    ...
    private static Class<?> getDateTimeType(GenerationConfig config) {
        Class<?> type = tryLoadType(config.getDateTimeType(), "date-time");
        if (type != null) {
            return type;
        }
        return config.isUseJodaDates() ? DateTime.class : LocalDateTime.class;
    }

    private static Class<?> getDateType(GenerationConfig config) {
        Class<?> type = tryLoadType(config.getDateType(), "date");
        if (type != null) {
            return type;
        }
        return config.isUseJodaLocalDates() ? LocalDate.class : LocalDateTime.class;
    }
